...
public class EstimationFrame extends javax.swing.JFrame implements ChartMouseListener {
    ...
    XYPlot scalingPlot;
    XYPlot otherPlot;
    HashMap<String, TreeMap<Double, Double>> otherCurves = new HashMap<String, TreeMap<Double, Double>>();
    ...
    public EstimationFrame(JFrame frm, EstimationFactory estimFac) {
        ...
        smoothSpinner.setEnabled(estim.getCurve().size() > 50);
    ...
    private void smoothSpinnerStateChanged(javax.swing.event.ChangeEvent evt) {//GEN-FIRST:event_smoothSpinnerStateChanged
        ...
        try {
//            curve[1] = new LoessInterpolator((Double)smoothSpinner.getValue(),
//                    LoessInterpolator.DEFAULT_ROBUSTNESS_ITERS)
//                    .smooth(curve[0], curve[1]);

            double bandwidth = (Double)smoothSpinner.getValue();
            if(bandwidth > 0)
                curve = convolve(curve, bandwidth, estim.getType() == EstimationFactory.Type.LOG);
        } catch (Exception ex) {
            Logger.getLogger(EstimationFrame.class.getName()).log(Level.SEVERE, null, ex);
        }
